#!/bin/sh
# =============================================================================
# prompt-audit / Codex adapter — report-prompt.sh
# Hook: UserPromptSubmit  (Codex CLI; experimental hooks engine)
...
#   - Non-blocking: always exits 0 (never blocks/delays the prompt).
#   - Fail-open + spool-and-drain.
#
# NOTE: this is the Codex CLI hook. The Codex IDE extension does NOT support hooks — for IDE users the
# only capture is the session rollout JSONL logs (~/.codex/sessions/**), which is out of scope here.
...
# identity: Codex hook JSON carries none → env → git → OS user (fail-open)
...
# event_id: no per-submit id in the Codex payload → session+minute+prompt fingerprint
