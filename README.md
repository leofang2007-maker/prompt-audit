...
| `GET` | `/api/v1/auth/oidc/login` · `/callback` · `/status` | — | Native OIDC SSO (spec [0008](docs/specs/0008-oidc-sso-rbac.md)): authorization-code flow with full id_token verification; identity → role/tenant mapping (**default-deny**). Off unless `app.oidc.enabled`. |
| `GET` | `/api/v1/auth/me` | admin | Profile for the current session (hydrates the SPA after an SSO redirect). |
...
| `GET` | `/api/v1/evidence?from=&to=` | auditor | Audit-ready **evidence pack** (spec [0007](docs/specs/0007-audit-ready-kit.md)): integrity + access-log summary + coverage + redaction stats + counts + config attestation, anchored by a `bundle_hash`. Summaries/hashes only, no prompt text. Tenant-scoped, access-logged. |
...
- ✅ **[Audit-ready kit](https://github.com/leofang2007-maker/prompt-audit/issues/6)** *(shipped)* — [SOC 2 / ISO 27001 control mappings](docs/compliance/) + a tamper-anchored **evidence pack** (`GET /api/v1/evidence`) an auditor can generate, view, and download. Design: [spec 0007](docs/specs/0007-audit-ready-kit.md).
- ✅ **[SSO + finer RBAC](https://github.com/leofang2007-maker/prompt-audit/issues/7)** *(shipped)* — native **OIDC** (Okta / Azure AD / Google / any OIDC IdP) with full id_token verification, and **default-deny** identity→role/tenant mapping. Design: [spec 0008](docs/specs/0008-oidc-sso-rbac.md). (SAML: delegate to an OIDC-capable IdP or a proxy.)
